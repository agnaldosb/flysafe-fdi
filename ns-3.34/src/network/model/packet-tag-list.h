...
public:
   ...
   * @author Vinicius - MiM
   * @date Nov 18, 2025
   * @brief Add a tag with encryption to the head of this branch
   * @param tag the tag to add
   * @param key the encryption key
   * @param nonce the encryption nonce
   */
  void Add (Tag const&tag, const std::string& key, const std::string& nonce) const;
  /**
   ...
   * @author Vinicius - MiM
   * @date Nov 18, 2025
   * @brief Remove (the first instance of) tag with encryption from the list
   * @param tag the tag to remove
   * @param key the encryption key
   * @param nonce the encryption nonce
   * @returns true if the requested tag is found, false otherwise
   */
  bool Remove (Tag &tag, const std::string& key, const std::string& nonce);
  /**
   ...
   * @author Vinicius - MiM
   * @date Nov 18, 2025
   * @brief Replace the value of a tag with encryption
   * @param tag The tag type to replace
   * @param key the encryption key
   * @param nonce the encryption nonce
   * @returns true if the requested tag is found, false otherwise
   */
  bool Replace (Tag &tag, const std::string& key, const std::string& nonce);
  /**
   ...
   * @author Vinicius - MiM
   * @date Nov 18, 2025
   * @brief Find a tag with encryption and return its value
   * @param tag The tag type to replace
   * @param key the encryption key
   * @param nonce the encryption nonce
   * @returns true if the requested tag is found, false otherwise
   */
  bool Peek (Tag &tag, const std::string& key, const std::string& nonce) const;
  /**
...
private:
   ...
   * @author Vinicius - MiM
   * @date Nov 18, 2025
   * @brief Traverse the list implementing copy-on-write, using Writer
   * @param tag The tag type to operate on
   * @param Writer The copy-on-write function to use
   * @param key the encryption key
   * @param nonce the encryption nonce
   * @returns true if the requested tag is found, false otherwise
   */
  ...
  */
   ...
   * @author Vinicius - MiM
   * @date Nov 18, 2025
   * @brief Copy-on-write implementing Remove
   * @param tag The target tag type to remove
   * @param preMerge True if tag was found before the first merge, false otherwise
   * @param cur Pointer to the tag
   * @param prevNext Pointer to the struct TagData.next pointer pointing to cur
   * @param key the encryption key
   * @param nonce the encryption nonce
   * @returns True, since tag will definitely be removed
   */
  ...
  */
   ...
   * @author Vinicius - MiM
   * @date Nov 18, 2025
   * @brief Copy-on-write implementing Replace
   * @param tag The target tag type to remove
   * @param preMerge True if tag was found before the first merge, false otherwise
   * @param cur Pointer to the tag
   * @param prevNext Pointer to the struct TagData.next pointer pointing to cur
