...
void 
Packet::AddPacketTag (const Tag &tag, std::string& key, std::string& nonce) const
{
  NS_LOG_FUNCTION (this << tag.GetInstanceTypeId ().GetName () << tag.GetSerializedSize ());
  m_packetTagList.Add (tag, key, nonce);
}
...
// Vinicius - MiM - 18 Nov, 2025
bool 
Packet::RemovePacketTag (Tag &tag, std::string& key, std::string& nonce)
{
  NS_LOG_FUNCTION (this << tag.GetInstanceTypeId ().GetName () << tag.GetSerializedSize ());
  bool found = m_packetTagList.Remove (tag, key, nonce);
  return found;
}
...
// Vinicius - MiM - 18 Nov, 2025
bool
Packet::ReplacePacketTag (Tag &tag, std::string& key, std::string& nonce)
{
  NS_LOG_FUNCTION (this << tag.GetInstanceTypeId ().GetName () << tag.GetSerializedSize ());
  bool found = m_packetTagList.Replace (tag, key, nonce);
  return found;
}
...
// Vinicius - MiM - 18 Nov, 2025
bool 
Packet::PeekPacketTag (Tag &tag, std::string& key, std::string& nonce) const
{
  bool found = m_packetTagList.Peek (tag, key, nonce);
  return found;
}
