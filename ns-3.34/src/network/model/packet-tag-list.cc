...
// Vinicius - MiM - 18 Nov, 2025
bool
PacketTagList::COWTraverse (Tag & tag, PacketTagList::COWWriter Writer)
{
  std::string empty = "";
  return COWTraverse(tag, Writer, empty, empty);
}
bool
PacketTagList::COWTraverse (Tag & tag, PacketTagList::COWWriter Writer, 
                            const std::string& key, const std::string& nonce)
{
  ...
  while (cur != 0)
    {
      if (cur->count > 1)
        ...
        {
          NS_LOG_INFO ("found tid before initial merge, calling writer");
          found = (this->*Writer)(tag, true, cur, prevNext, key, nonce);
          break;
        }
      else
        {
  ...
  */
  ...
  // link around tid, removing it from our list
  found = (this->*Writer)(tag, false, cur, prevNext, key, nonce);
  return found;
...
// Vinicius - MiM - 18 Nov, 2025
bool
PacketTagList::Remove (Tag & tag, const std::string& key, const std::string& nonce)
{
  return COWTraverse (tag, &PacketTagList::RemoveWriter, key, nonce); 
}
                             ...
                             const std::string& key, const std::string& nonce)
{
  ...
  if (key.empty() && nonce.empty()) {
      tag.Deserialize (TagBuffer (cur->data, cur->data + cur->size));
  } else {
      tag.Deserialize (TagBuffer (cur->data, cur->data + cur->size), key, nonce);
  }
...
// Vinicius - MiM - 18 Nov, 2025
bool
PacketTagList::Replace (Tag & tag, const std::string& key, const std::string& nonce)
{
  bool found = COWTraverse (tag, &PacketTagList::ReplaceWriter, key, nonce); 
  if (!found)
    {
      Add (tag, key, nonce); 
    }
  ...
  if (preMerge)
    {
      ...
      if (key.empty() && nonce.empty()) {
          ...
          tag.Serialize (TagBuffer (cur->data, cur->data + cur->size), key, nonce);
      }
    }
  else
    {
      ...
      uint32_t newSize;
      if (key.empty() && nonce.empty()) {
          newSize = tag.GetSerializedSize();
      } else {
          newSize = tag.GetSerializedSize(key, nonce);
      }

      struct TagData * copy = CreateTagData (newSize);
      copy->tid = tag.GetInstanceTypeId ();
      copy->count = 1;
      ...
      if (key.empty() && nonce.empty()) {
          ...
          tag.Serialize (TagBuffer (copy->data, copy->data + copy->size), key, nonce);
      }
...
// Vinicius - MiM - 18 Nov, 2025
void
PacketTagList::Add (const Tag &tag, const std::string& key, const std::string& nonce) const
{
  NS_LOG_FUNCTION (this << tag.GetInstanceTypeId ());
  // ensure this id was not yet added
  for (struct TagData *cur = m_next; cur != 0; cur = cur->next)
  ...
  struct TagData * head = CreateTagData (tag.GetSerializedSize (key, nonce));
  head->count = 1;
  head->next = 0;
  head->tid = tag.GetInstanceTypeId ();
  head->next = m_next;
  tag.Serialize (TagBuffer (head->data, head->data + head->size), key, nonce);
...
// Vinicius - MiM - 18 Nov, 2025
bool
PacketTagList::Peek (Tag &tag, const std::string& key, const std::string& nonce) const
{
  NS_LOG_FUNCTION (this << tag.GetInstanceTypeId ());
  TypeId tid = tag.GetInstanceTypeId ();
  for (struct TagData *cur = m_next; cur != 0; cur = cur->next)
